...
python3 tools/inspector.py <pokernow_game_url>
```
...
```
DOM (Playwright) → browser/scraper.py → GameState → engine.py → (action, amount) → browser/actions.py → clicks
                                                       ↑
...
```
.
├── main.py                  Game loop, session lifecycle
├── engine.py                Per-street decision orchestration
├── session_logger.py        Decisions / hands / console log writers
├── ledger_fetch.py          Pulls pokernow ledger CSV
├── anonymize.py             Stable opponent-name hashing
├── browser/                 DOM I/O (fragile pokernow selectors live here)
│   ├── scraper.py
│   ├── actions.py
│   └── state.py             Card / GameState dataclasses
├── decision/                Pure poker logic — no I/O, no Playwright
│   ├── hand_classifier.py
│   ├── preflop_ranges.py
│   ├── turn_heuristic.py
│   ├── river_heuristic.py
│   ├── heuristic_utils.py   Shared turn/river constants + helpers
│   ├── bet_sizing.py
│   ├── solver_lookup.py
│   └── ranges.py
├── tracking/                Per-hand and session-persistent observations
│   ├── hand_tracker.py
│   ├── hand_context.py
│   └── opponent_profiles.py
├── tools/                   CLI debugging utilities (not imported by main)
│   ├── inspector.py
│   ├── inspector_chat.py
│   └── inspector_ledger.py
└── scripts/                 Offline jobs (presolve, build lookup, deploy, etc.)
```
...
Constants and helpers shared by both streets (`IP_POSITIONS`, `STRONG`, `FOLD_SENSITIVE`, `TIER_ADJ`, `bet_tier`, `clamp`, `normalize`) live in `decision/heuristic_utils.py`. River differences from turn: no semi-bluffs (draws resolved), higher base fold on marginal hands, air bluffs at 20-25%, `trips_board` texture, SPR thresholds at <2 and >8 instead of <4 and >10.

**`decision/bet_sizing.py`** provides `pick_bet_fraction(hand_class, board_cards, facing_bet)` and `pick_preflop_size(position, facing_raise, to_call, bb)`. Postflop samples from {0.33, 0.66, 1.00, 1.50} × pot using hand-class weight tables modified by board wetness. Air and monster/nutted hands are both weighted toward larger sizes (polarised sizing). Preflop samples from BB multiples by position.

**`tracking/hand_tracker.py`** infers opponent range from state transitions (not an action stream). Preflop: `to_call / big_blind` ratio maps to one of five tiers. Postflop: each street where `to_call > 0` tightens the tier one step toward `premium`. Reset at start of each new hand.

**`decision/ranges.py`** defines five hand range tiers as sets of canonical `(high_rank, low_rank, suited)` tuples. Expanded into full specific-combo lists at module load. `get_combos(tier, exclude)` filters out known cards before returning the pool for Monte Carlo sampling.

**`browser/actions.py`** handles raise panel interaction: clicks `button.raise` → types amount into `.raise-bet-value input[type='text']` (using `Meta+a` + `press_sequentially` to trigger React's onChange) → submits. Falls back to the closest preset button if the text input isn't found. Falls back to Min Raise if the submitted amount is rejected by the site.
...
pokernow is a React app and class names change between site updates. All selectors live in `browser/scraper.py` at the top of the file. If the bot stops reading cards, pot, or buttons correctly, run `python3 tools/inspector.py` to dump the live DOM and compare against the selectors. The raise input selectors (`SEL_RAISE_INPUT`, `SEL_RAISE_SUBMIT`) are in `browser/actions.py` and are separately fragile — the inspector prompts you to open the raise panel manually so it can dump those too.

## Session artifacts

**`tracking/hand_context.py`** — per-hand memory. Tracks our actions per street, infers villain activity from state deltas, records who the preflop aggressor is, exposes `hero_cbet(street)` / `is_double_barrel_spot(street)` / `villain_check_called_last_street(street)` queries used by the turn/river heuristics. Reset on every new hand.

**`tracking/opponent_profiles.py`** — session-persistent registry keyed by anonymized username. Tracks VPIP, PFR, AF, 3bet%, fold-to-cbet% across hands. Saves/loads `Logs/profiles.json` so profiles survive across bot runs. Derives a tier live from the accumulated stats; blended with the per-hand tightener in `main.py` via `_blend_tier()` (takes whichever is tighter).
...
**After any code change you make**, bump the version, commit, and push — **always, without asking**. This is standing authorization for every code change in this repo. Any time you have modified one or more files, run the full sequence below before considering the task complete. Do not stop after editing; do not ask "should I commit?"; do not skip the push.
